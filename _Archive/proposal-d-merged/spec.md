# Blocks Markdown — merged proposal

> **Status:** **Promoted 2026-09-10** — this proposal was adopted as the working spec at
> [`../../md-blocks-spec.md`](../../md-blocks-spec.md). This folder is kept as the historical record of the proposal;
> edit the spec at `../../md-blocks-spec.md`, not here.
>
> ~~Proposal, revision 1 — 2026-09-09.~~ Merge of
> [C (Claude, author-oriented)](../proposal-c-claude/spec.md) and
> [B (Astra, editor-oriented)](../proposal-b-astra/spec.md), informed by the
> [authoring test](../test-runs/REPORT.md).
> **Companion:** [showcase.md](showcase.md) — the showcase *is* the tutorial; this file is the rulebook
> for whoever writes the parser and the editor.
...
That is the authoring surface. What distinguishes this proposal is §7: **what the editor is required to
...
4. **Editor-safe.** Every visual-editor operation has exactly one serialization, and reparsing it yields
   ...
   editor (§7) — never reconstructed by guessing.
...
|---|---|---|---|
| Document metadata | YAML frontmatter | `bm:document` JSON | **frontmatter** (C) |
| Section separator | `---` thematic break | root H1 / `bm:section` | **`---`** (C) |
| Block | explicit, closes | explicit, closes | **C = B** (they agreed) |
| Media | block whose first node is media | `bm:media` + strict body + `caption=` | **C's shape** + B's strictness via editor-stamped `kind` (§7.2) |
...
| Unannotated-Markdown chunking | "editor policy, not format" | maximal run = one implicit block | **B's rule is format law** (§7.1) |
| Block identity | optional `id` | optional `id` | **editor stamps `id` on save** (§7.3) |
| Type reclassification | silent (first-node inference) | impossible (explicit `kind`) | **loud**: stamped `kind` contradicting the body is an error (§7.2) |
| Validation | strict table | strict table | **merged** (§9) |
| Round-trip | invariant stated | invariant + preservation list + trust boundary | **B's, adapted** (§10) |
...
- **Section separator:** a **root-level thematic break** (`---`, `***`, `___` per CommonMark; canonical
  ...
  validator emits a warning for paragraph-line + `---` with no intervening blank line (§9).
...
|---|---|---|
| `id` | identifier | Optional stable identity, unique per file. Anchor target in enhanced output. Editor-stamped when absent (§7.3). |
...
- Unannotated Markdown outside any block is chunked by the **deterministic rule in §7.1** — identical on
...
save** (§7.2). Once stamped, it is authoritative: if the body stops matching the stamped `kind`, that is a
...
kind* or *attribute*: parse error. The legacy CMS's composite palette entries are editor templates that
expand into these primitives; they are not vocabulary.
...
## 7. The editor contract (what distinguishes this proposal)

C left three things implicit that a visual editor cannot afford to guess. This proposal keeps C's
...
### 7.1 Unannotated Markdown: one rule, not editor policy
...
### 7.2 Media `kind`: inferred when authored, stamped when saved
...
### 7.3 Stable identity: ids stamped, never derived
...
### 7.4 What the editor never does
...
| Stamped `kind` contradicting the block's first node (§7.2) | error at block marker, citing stamped kind |
...
2. ~~**Unannotated chunking.**~~ Decided 2026-09-09: maximal run, format law (§7.1). C had left it to
...
4. **Prefix.** `bm:` is inherited. Only the *existence* of a prefix is load-bearing.
5. **Frontmatter schema.** Which keys the CMS profile requires is a profile decision, not a format one.

## 11. Conformance cases (to turn into fixtures)
...
| Maximal unannotated run with blank lines, heading, list inside | one implicit markdown block (§7.1) |
